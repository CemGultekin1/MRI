function CRBvals=grid_sample(name,sample_num1,sample_num2,control,pastver)
TR=3.5e-3;
TRFmax=5e-4;
...
if pastver==1
    ...
    for j=1:length(past_versions)
        ...
        parfor i=1:sample_num
             [~,C(:,i),~]=Bloch.simulate_multishooting(samples{i},TR1,TRFmax1,sweep_phase,control1,order);
        end
        CRBs{j}=C;
    end
else
